...
P.path = matlab.desktop.editor.getActiveFilename ;  
idx = strfind(P.path,'\') ;
P.path = P.path(1:idx(end-1)) ;
P.scene_in = fullfile(P.path,'data','scene_images','SDesc01') ; 
P.patch_out = {fullfile(P.path,'data','patch_stimuli','sdesc01fine')
               fullfile(P.path,'data','patch_stimuli','sdesc01coarse')} ;
P.catch_out = fullfile(P.path,'data','patch_stimuli','catch',...
                       'default') ;
...
P.scene_context = 0 ;                % 0=patch only, 1=patch in context
P.context_color = [57 255 20] ;      % Color used to highlight patch
P.context_scale = .25 ;              % Scale down context scene size
...
for k=1:length(P.patch_diameter)
    ...
    %--Save to data folder
    saveas(gcf,[fullfile(P.path,'data') filesep ...
