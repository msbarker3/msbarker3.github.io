...
P.scene_in = fullfile(P.path,'data','scene_images',expr) ; 
P.patch_in = {fullfile(P.path,'data','patch_data',expr,task,'fine')
              fullfile(P.path,'data','patch_data',expr,task,'coarse')} ;
P.pixel_out = {fullfile(P.path,'data','pixel_data',expr,task,'fine')
               fullfile(P.path,'data','pixel_data',expr,task,'coarse')} ;
P.pixel_out_combined = fullfile(P.path,'data','pixel_data',expr,task,'combined');
...
for p=1:length(P.patch_scale)
    ...
    for k=1:length(s_filenames)
        ...
        patch.img_name = strrep(s_filenames{k},'.mat','.jpg') ;
        ...
        patch_pixel_match(strrep(s_filenames{k},'.mat',''),patch) ;
        fprintf('  %s complete\n',s_filenames{k}) ;       
